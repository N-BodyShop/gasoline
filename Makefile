...
#CODE_DEF += -DALPHACMUL=0.1 #The AV coeffcient multiplier
#CODE_DEF += -DALPHAMIN=0.01 #The minimum AV coefficent
...
#CODE_DEF += -DCONTROLYHII #Make sure error in YHII and YHeIII is controlled
#CODE_DEF += -DCOOLING_METAL_BROKEN #Debug lines for outputting some extra data for debugging cooling_metal
#CODE_DEF += -DCOOLDEBUG #Output debugging information from the gas cooling
...
#CODE_DEF += -DFEEDBACKDIFFLIMIT #Disable thermal diffusion for particles that have their cooling shut off
...
#CODE_DEF += -DGLASSZ #Use this to make an IC that has a glassy distribution in the z-axis
...
#CODE_DEF += -DGRADWONESIDED #Only use the primary particle for calculating the kernel gradient (one-sided)
#CODE_DEF += -DGROWMASS #Allow collisional particles to grow
#CODE_DEF += -DHSHRINK #M4 kernel times (pi/6)^(1/3) for a tighter kernel
#CODE_DEF += -DINELASTICCOLLAPSE #Use the experimental inellastic collaspse strategy for collisions
...
#CODE_DEF += -DNODIFFUSIONTHERMAL #Disable all thermal diffusion
...
#CODE_DEF += -DOLDINITSPHCODE #Use the older multi-resmooth msrInitSph (deprecated?)
#CODE_DEF += -DOLD_COMET_FORCE #Enable the do_force for special rubble pile particles (ie comets).
#CODE_DEF += -DOLD_COMET_STUFF #Initialize special particles (this is a huge kludge)
...
#CODE_DEF += -DPCONST=? #Use a constant pressure specified for all gas
...
#CODE_DEF += -DPREDRHO #Generate predicted rho values (if bPredRho == 2)
...
#CODE_DEF += -DPONRHOFLOOR=1e-30 #Minimum P/rho value, prevents floating point errors in some cases
#CODE_DEF += -DQUINTIC #Use a quintic spline kernel (wants 108-180 neighbours) from Dehnen & Aly 2012 (Bibcode:2012MNRAS.425.1068D)
#CODE_DEF += -DRANOLD #Use an older random number generator for debugging the rubble code
...
#CODE_DEF += -DSFBOUND #Use a surface-density criteria for allowing star formation.
...
#CODE_DEF += -DSNIA_TST #Print out SN Ia feedback properties
#CODE_DEF += -DSPECIAL_PARTICLES #Include support for weird special particles (not star, gas, or DM)
#CODE_DEF += -DSPH1D #Only allow forces in the x-direction
#CODE_DEF += -DSPINUP #Use alternative rotating frame, just apply accelerations.
#CODE_DEF += -DSTARCLUSTERFORM #Make new stars using the clustered star formation recipe
...
#CODE_DEF += -DWOLFIRE #Use the Wolfire values for the metal cooling rate with UV heating
#CODE_DEF += -DWOLFIRE_NOTWOPHASE #Use an imposed cooling rate without a two-phase instability
