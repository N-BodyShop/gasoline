...
int mdlInitialize(MDL *pmdl,char **argv,void (*fcnChild)(MDL))
{
	...
	*pmdl = NULL;
	...
	/* First MDL_N_RECV_BUFFER are recv buffers 
	   Next MDL_N_FLSH_BUFFER are flsh buffers */
	for (i=0;i<MDL_N_CACHE_BUFFER;i++) {
	  mdl->ReqBuffer[i] = malloc(mdl->iCaBufSize);
   	  assert(mdl->ReqBuffer[i] != NULL);
	  mdl->bReqActive[i] = 0;
	}
	mdl->ReqHandle = malloc(sizeof(MPI_Request)*MDL_N_CACHE_BUFFER);
    assert(mdl->ReqHandle != NULL );

	mdl->iRecvCurrent = -1;  /* Buffer with data received -- none */
	mdl->nRecvActive = 0;
	mdl->iFlshCurrent = MDL_FIRST_FLSH_BUFFER;  /* Buffer to use -- first */
	mdl->nFlshActive = 0;  
...
void mdlFinish(MDL mdl)
{
  int i;
	...
	free(mdl->cache);
	free(mdl->ReqHandle);
	for (i=0;i<MDL_N_CACHE_BUFFER;i++) free(mdl->ReqBuffer[i]);
...
/* Assumes there is a new message with data pointed to by mdl->pszRcv 
   Switching to assuming message is ReqBuffer[mdl->iRecvCurrent] */
int mdlCacheReceive(MDL mdl,char *pLine)
{
	CACHE *c;
	CAHEAD *ph = (CAHEAD *)(mdl->ReqBuffer[mdl->iRecvCurrent]);
	char *pszRcv = &(mdl->ReqBuffer[mdl->iRecvCurrent][sizeof(CAHEAD)]);
	CAHEAD *phRpl;
	char *pszRpl;
	char *t;
	int id, iTag;
	...
	MPI_Irecv(mdl->ReqBuffer[mdl->iRecvCurrent],mdl->iCaBufSize, MPI_BYTE, id,
		 iTag, MPI_COMM_WORLD, &mdl->ReqHandle[mdl->iRecvCurrent] );
	mdl->nRecvActive ++;
...
void AdjustDataSize(MDL mdl)
{
	...
	if (iMaxDataSize != mdl->iMaxDataSize) {
		...
		*/
		/* Or just mpi_cancel ... */
		for (i=MDL_FIRST_RECV_BUFFER;i<=MDL_LAST_RECV_BUFFER;i++) {
		  ret = MPI_Cancel(&mdl->ReqHandle[i] );
		  assert( ret == MPI_SUCCESS );
		  mdl->nRecvActive--;
		}
		assert(mdl->nRecvActive == 0);

		mdl->iMaxDataSize = iMaxDataSize;
		mdl->iCaBufSize = sizeof(CAHEAD) + 
			iMaxDataSize*(1 << MDL_CACHELINE_BITS);
		for (i=0;i<MDL_N_FLSH_BUFFER;i++) {
		  mdl->ReqBuffer[i] = realloc(mdl->ReqBuffer[i],mdl->iCaBufSize);
		  assert(mdl->ReqBuffer[i] != NULL);
		}
		for (i=MDL_FIRST_RECV_BUFFER;i<=MDL_LAST_RECV_BUFFER;i++) {
		  mdl->ReqBuffer[i] = realloc(mdl->ReqBuffer[i],mdl->iCaBufSize);
		...
		/*
		 * Need to shut these down in advance and then restart
		 * Fire up receive again.
		 */
		for (i=MDL_FIRST_RECV_BUFFER;i<=MDL_LAST_RECV_BUFFER;i++) {
		  MPI_Irecv(mdl->ReqBuffer[i],mdl->iCaBufSize, MPI_BYTE,
			  MPI_ANY_SOURCE, MDL_TAG_CACHECOM,
			  MPI_COMM_WORLD, &mdl->ReqHandle[i]);
		  mdl->nRecvActive++;
		  }
		}
	}
...
void mdlROcache(MDL mdl,int cid,void *pData,int iDataSize,int nData)
{
	...
	if(mdl->idSelf == 0) {
	    ...
	    while(c->nCheckIn < mdl->nThreads) {
		  ...
		  ret = MPI_Waitany( MDL_N_RECV_BUFFER, mdl->ReqHandle, &mdl->iRecvCurrent, &status);
		  assert(ret == MPI_SUCCESS);
		  mdl->nRecvActive--;
...
//		  ret = MPI_Wait(&mdl->ReqRcv, &status); 
		 ...
		 */
        /* JW Should we check for a pending flush? */
	    for (i=0;i < MDL_N_FLSH_BUFFER; i++) {
		  caFlsh = (CAHEAD *) &(mdl->ReqBuffer[i]);
  		  caFlsh->cid = cid;
		...
		for (i=1;i<c->nLines;++i) {
			iKey = c->pTag[i].iKey;
			if (iKey >= 0) {
			    caFlsh = (CAHEAD *) &mdl->ReqBuffer[mdl->iFlshCurrent];
				pszFlsh = &(mdl->ReqBuffer[mdl->iFlshCurrent][sizeof(CAHEAD)]);
				/*
				 ...
				 */
				assert(!mdl->bReqActive[mdl->iFlshCurrent]);
				MPI_Issend(caFlsh, sizeof(CAHEAD)+c->iLineSize,
					 MPI_BYTE, id, MDL_TAG_CACHECOM,
					 MPI_COMM_WORLD, &mdl->ReqHandle[mdl->iFlshCurrent] ); 
				mdl->bReqActive[mdl->iFlshCurrent] = 1;
				mdl->nFlshActive++;
				/*
				...
				*/
				...
				if (mdl->nFlshActive < MDL_N_FLSH_BUFFER && i<c->nLines-1 ) {
				  mdl->iFlshCurrent++; /* Use all flsh buffers before waiting */
				  assert(mdl->iFlshCurrent <= MDL_LAST_FLSH_BUFFER);
				  continue;
				}
				while(1) {
				    ret = MPI_Waitany(MDL_N_CACHE_BUFFER, mdl->ReqHandle, &index, &status);
					assert(ret == MPI_SUCCESS);
					/*				    assert(!(index != 0 && reqBoth[0] ==
										MPI_REQUEST_NULL)); */
				    if(index >= MDL_FIRST_FLSH_BUFFER && index <= MDL_LAST_FLSH_BUFFER) { /* Flush has completed */
					  mdl->iFlshCurrent = index; /* reuse this buffer next */
					  mdl->bReqActive[mdl->iFlshCurrent] = 0;
					  mdl->nFlshActive--;
/* JW Only clear one flush buffer at a time for now unless at end */
					  if (i<c->nLines-1 || mdl->nFlshActive==0) break; 
					}
				    else if (index >= MDL_FIRST_RECV_BUFFER && index <=  MDL_LAST_RECV_BUFFER) {
					  mdl->iRecvCurrent = index;
					  /* JW - Redundant? Perhaps to make up for prior error */
					  /*					  ret = MPI_Wait(&mdl->ReqRcv, &status);
											  assert(ret == MPI_SUCCESS); */
					  mdl->nRecvActive --;
					  mdlCacheReceive(mdl, NULL);
	...
	/* This is a confirmation from the master node that all nodes are done.
	   At this stage some other node may still be working and the
	   ability to service requests must be maintained until confirmation recvd */
	if(mdl->idSelf == 0) {
	    for(id = 1; id < mdl->nThreads; id++) {
		  MPI_Send(&caOut,sizeof(CAHEAD),MPI_BYTE, id,
			       MDL_TAG_CACHECOM, MPI_COMM_WORLD);
	...
	/* JW: Need to cancel cache recv buffers? */
	 ...
	 * Cache is shut down if NOCACHE type which is detector for last cache
	 */
	if(last) {
	    assert(mdl->nRecvActive == MDL_N_RECV_BUFFER);
		for (i=MDL_FIRST_RECV_BUFFER;i<=MDL_LAST_RECV_BUFFER;i++) {
		  ret = MPI_Cancel(&mdl->ReqHandle[i] );
		  assert (ret == MPI_SUCCESS);
		  mdl->nRecvActive--;
		}
		assert(mdl->nRecvActive == 0); /* No recv's active */
	    }
...
void mdlCacheCheck(MDL mdl)
{
	...
	/* JW: MPI_Testsome may be more efficient here */
    while (1) {
	  /*	MPI_Test(&mdl->ReqRcv, &flag, &status); */
	  MPI_Testany( MDL_N_RECV_BUFFER, mdl->ReqHandle, &mdl->iRecvCurrent, &flag, &status); 
  	  if(flag == 0) break;
  	  mdlCacheReceive(mdl,NULL);
      }
    }
...
GotVictim:
	...
	if (iKeyVic >= 0) {
		if (c->iType == MDL_COCACHE) {
			...
			for(i = 0; i < c->iLineSize; ++i) pszFlsh[i] = pLine[i];

			mdl->bReqActive[mdl->iFlshCurrent]=1;
			MPI_Isend(caFlsh, sizeof(CAHEAD)+c->iLineSize,
				 MPI_BYTE, idVic,
				 MDL_TAG_CACHECOM, MPI_COMM_WORLD, &mdl->ReqHandle[mdl->iFlshCurrent]); 
			mdl->nFlshActive++;
			}
		/*
	...
	*pi = iVictim;
	...
	while (1) {
	  ...
	  if (index >= MDL_FIRST_RECV_BUFFER && index <= MDL_LAST_RECV_BUFFER) {
		mdl->nRecvActive--;
		mdl->iRecvCurrent = index;
		mdlCacheReceive(mdl,pLine);
	    return(&pLine[iElt*c->iDataSize]);
	  }
	  /* Flush */
	  assert( index >= MDL_FIRST_FLSH_BUFFER && index <= MDL_LAST_FLSH_BUFFER );
	  mdl->iFlshCurrent = index;
	  mdl->bReqActive[mdl->iFlshCurrent] = 0;
	  mdl->nFlshActive--;
      }
	}
