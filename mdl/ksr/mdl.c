...
void BasicInit(MDL mdl)
{
     ...
     */
    mdl->mbxOwn.bRel = 0;
    pthread_mutex_init(&mdl->mbxOwn.mux,pthread_mutexattr_default);
    /*
    ...
    mdl->swxOwn.bSet = 0;
    pthread_mutex_init(&mdl->swxOwn.mux,pthread_mutexattr_default);
    }
...
void mdlFinish(MDL mdl)
{
    ...
    pthread_barrier_destroy(&mdl->bar);
    free(mdl->pmdl);
    free(mdl->pt);
    free(mdl);
    }
				...
				void (*init)(void *),void (*combine)(void *,void *))
{
    ...
    c->pMux = malloc(c->nMux*sizeof(pthread_mutex_t));
    assert(c->pMux != NULL);
    for (i=0;i<c->nMux;++i) {
		pthread_mutex_init(&c->pMux[i],pthread_mutexattr_default);
		}
    /*
     ** Must synchronize.
     */
...
void mdlFinishCache(MDL mdl,int cid)
{
    ...
    for (i=1;i<c->nLines;++i) {
		...
		if (iLine >= 0) {
			 ...
			 */
			pthread_mutex_lock(&cc->pMux[iLine]);
			for (j=0;j<n;j+=c->iDataSize) {
				(*c->combine)(&dest[j],&srce[j]);
				}
			pthread_mutex_unlock(&cc->pMux[iLine]);
			}
		}
    /*
     ** Must synchronize.
     ...
     */
    for (i=0;i<c->nMux;++i) {
		pthread_mutex_destroy(&c->pMux[i]);
		}
    /*
