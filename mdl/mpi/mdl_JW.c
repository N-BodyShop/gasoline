		...
		* I think this makes for bad synchronizes --trq
		...
		for (i=1;i<c->nLines;++i) {
			...
			if (iKey >= 0) {
				...
				*/
				/* JW This is potentially bad -- these may be 
				   copies of active data depending on the mpi implementation 
				   better to construct an array reqs for general purpose 
				*/
...
void mdlSyncCache(MDL mdl,int cid)
{
	CACHE *c = &mdl->cache[cid];
	CAHEAD caOut;
	char *pszFlsh = &mdl->pszFlsh[sizeof(CAHEAD)];
	...
	/*
	 ** THIS IS A SYNCHRONIZE!!!
	 ** use cache check outs to acknowledge synchronization
	 */
	caOut.cid = cid;
	caOut.mid = MDL_MID_CACHEOUT;
	caOut.id = mdl->idSelf;
	...
	/*
	 ** Clear checkout data -- it was temporary for synchronize
	 */
	c->nCheckOut = 0;
	}
