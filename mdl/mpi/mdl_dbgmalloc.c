...
#include "mpi.h"
#include "mdl.h"
#include "../../pkdgrav_lite/dbgmalloc.h"
...
int mdlInitialize(MDL *pmdl,char **argv,void (*fcnChild)(MDL))
{
	...
	*pmdl = NULL;
	mdl = DBGmalloc(sizeof(struct mdlContext),"mdl");
	assert(mdl != NULL);
	/*
	 ...
	 */
	mdl->psrv = DBGmalloc(mdl->nMaxServices*sizeof(SERVICE),"psrv");
	 ...
	 */
	mdl->pszIn = DBGmalloc(mdl->nMaxSrvBytes+sizeof(SRVHEAD),"pszIn");
	assert(mdl->pszIn != NULL);
	mdl->pszOut = DBGmalloc(mdl->nMaxSrvBytes+sizeof(SRVHEAD),"pszOut");
	assert(mdl->pszOut != NULL);
	mdl->pszBuf = DBGmalloc(mdl->nMaxSrvBytes+sizeof(SRVHEAD),"pszBuf");
	assert(mdl->pszBuf != NULL);
	/*
	 ** Allocate swapping transfer buffer. This buffer remains fixed.
	 */
	mdl->pszTrans = DBGmalloc(MDL_TRANS_SIZE,"pszTrans");
	assert(mdl->pszTrans != NULL);
	/*
	 ** Allocate initial cache spaces.
	 */
	mdl->cache = DBGmalloc(mdl->nMaxCacheIds*sizeof(CACHE),"cache");
	...
	mdl->pszRcv = DBGmalloc(mdl->iCaBufSize,"pszRcv");
	assert(mdl->pszRcv != NULL);
	mdl->ppszRpl = DBGmalloc(mdl->nThreads*sizeof(char *),"ppszRpl");
	assert(mdl->ppszRpl != NULL);
	mdl->pmidRpl = DBGmalloc(mdl->nThreads*sizeof(int),"pmidRpl");
	assert(mdl->pmidRpl != NULL);
	for (i=0;i<mdl->nThreads;++i)
		mdl->pmidRpl[i] = -1;
	mdl->pReqRpl = DBGmalloc(mdl->nThreads*sizeof(MPI_Request),"pReqRpl");
	assert(mdl->pReqRpl != NULL);
	for (i=0;i<mdl->nThreads;++i) {
		mdl->ppszRpl[i] = DBGmalloc(mdl->iCaBufSize,"ppszRpl");
		assert(mdl->ppszRpl[i] != NULL);
		}
	mdl->pszFlsh = DBGmalloc(mdl->iCaBufSize,"pszFlsh");
...
void mdlFinish(MDL mdl)
{
  int i;
	 ...
	 ** Deallocate storage.
	 */
	DBGfree(mdl->psrv,"psrv");
	DBGfree(mdl->pszIn,"pszIn");
	DBGfree(mdl->pszOut,"pszOut");
	DBGfree(mdl->pszBuf,"pszBuf");
	DBGfree(mdl->pszTrans,"pszTrans");
	DBGfree(mdl->cache,"cache");
	DBGfree(mdl->pszRcv,"pszRcv");
	DBGfree(mdl->pszFlsh,"pszFlsh");
	for (i=0;i<mdl->nThreads;++i) DBGfree(mdl->ppszRpl[i],"ppszRpl");
	DBGfree(mdl->ppszRpl,"ppszRpl");
	DBGfree(mdl->pmidRpl,"pmidRpl");
	DBGfree(mdl->pReqRpl,"pReqRpl");
	DBGfree(mdl,"mdl");
	}
				   ...
				   int nInBytes,int nOutBytes)
{
	...
	if (sid >= mdl->nMaxServices) {
		 ...
		 */
		nMaxServices = sid + 9;
		mdl->psrv = DBGrealloc(mdl->psrv,nMaxServices*sizeof(SERVICE),"psrv");
		assert(mdl->psrv != NULL);
		/*
		 ** Initialize the new service slots.
		 */
	...
	if (nMaxBytes > mdl->nMaxSrvBytes) {
		mdl->pszIn = DBGrealloc(mdl->pszIn,nMaxBytes+sizeof(SRVHEAD),"pszIn");
		assert(mdl->pszIn != NULL);
		mdl->pszOut = DBGrealloc(mdl->pszOut,nMaxBytes+sizeof(SRVHEAD),"pszOut");
		assert(mdl->pszOut != NULL);
		mdl->pszBuf = DBGrealloc(mdl->pszBuf,nMaxBytes+sizeof(SRVHEAD),"pszBuf");
		assert(mdl->pszBuf != NULL);
...
void AdjustDataSize(MDL mdl)
{
	...
	if (iMaxDataSize != mdl->iMaxDataSize) {
		...
		mdl->pszRcv = DBGrealloc(mdl->pszRcv,mdl->iCaBufSize,"pszRcv");
		assert(mdl->pszRcv != NULL);
		for (i=0;i<mdl->nThreads;++i) {
			mdl->ppszRpl[i] = DBGrealloc(mdl->ppszRpl[i],mdl->iCaBufSize,"ppszRpl");
			assert(mdl->ppszRpl[i] != NULL);
			}
		mdl->pszFlsh = DBGrealloc(mdl->pszFlsh,mdl->iCaBufSize,"pszFlsh");
		assert(mdl->pszFlsh != NULL);
...
{	
	return(DBGmalloc(iSize,"mdlAlloc"));
	}
...
CACHE *CacheInitialize(MDL mdl,int cid,void *pData,int iDataSize,int nData)
{
	...
	if (cid >= mdl->nMaxCacheIds) {
		 ...
		 */
		nMaxCacheIds = cid + 3;
		mdl->cache = DBGrealloc(mdl->cache,nMaxCacheIds*sizeof(CACHE),"cache");
		assert(mdl->cache != NULL);
		/*
...
void mdlFinishCache(MDL mdl,int cid)
{
	 ...
	 */
	DBGfree(c->pTrans,"pTrans");
	DBGfree(c->pTag,"pTag");
	DBGfree(c->pbKey,"pbKey");
	DBGfree(c->pLine,"pLine");
	c->iType = MDL_NOCACHE;
...
void *mdlAquire(MDL mdl,int cid,int iIndex,int id)
{
	...
	*pi = iVictim;
	...
	if (iKey >= c->nKeyMax) {			/* !!! */
		nKeyNew = iKey+500;
		c->pbKey = DBGrealloc(c->pbKey,nKeyNew,"pbKey");
		assert(c->pbKey != NULL);
