...
void clPrintCoolFile( COOL *cl, PERBARYON *Y, RATE *Rate, double rho, double ZMetal, FILE *fp ) {
  ...
  fprintf(fp,"Low T cooling  %e\n",en_B * 
    (wTln0*RT0->Cool_LowT+wTln1*RT1->Cool_LowT)*cl->R.Cool_LowTFactor*0.001);
  fprintf(fp,"Photon Heat HI %e %e\n",Y->HI*cl->R.Heat_Phot_HI*Rate->Phot_HI, Y->HI*cl->R.Heat_Phot_HI*Rate->Phot_HI);
...
void clAbunds( COOL *cl, PERBARYON *Y, RATE *Rate, double rho, double ZMetal) {
  ...
  double yeMax;
  double rye,ye;
  double fHI,fHII,fHeI,fHeII,rfH,rfHe,yHI_old,yHeII_old;
  double Rate_Phot_HI;
  int i;  
  ...
  Y->HeII = yHeII;
  Y->HeIII = yHeIII;
  Y->Total = Y->e + yH + yHe + ZMetal/MU_METAL;/*Don't want to double count hydrogen atoms in molecules when finding the total number of particles */
}
...
{
 double T,TA,TB;
 double Y_Total0 = (d->Y_H+d->Y_He)*.9999; 
...
void clDerivs(void *Data, double x, double *y, double *dydx) {
  ...
  d->E = y[1];
  d->Y.HI = y[2];
  d->Y.HII = d->Y_H - d->Y.HI; /*Don't forget about molec H CC*/
  ...
  d->Y.Total = d->Y.e + d->Y_H + d->Y_He + d->ZMetal/MU_METAL;  /* H total from cl now -- in future from particle */ 
