...
void BHSinkAccrete(PARTICLE *p,int nSmooth,NN *nnList,SMF *smf)
{
	...
	double aFac, dCosmoDenFac,dCosmoVel2Fac,aDot;
	FLOAT dvmin, dvx,dvy,dvz,dvv,dvcosmo;
	int ivmin;
...
//		  p->fDivv_Corrector = -3/(divvbad*fNorm1);
	p->fDivv_t = (divvnorm != 0 ? -3*trace/divvnorm : 0); /* physical -- includes H */
#ifdef DRHODT
	/* Hack to initialize */
...
void DistESF(PARTICLE *p,int nSmooth,NN *nnList,SMF *smf)
{
	...
	double ESFRate = p->fMass*smf->dESFEnergy/smf->dESFEndTime;
	printf("DEBUGES1: %e\n", ESFRate);
	ih2 = 4.0/BALL2(p);
...
#ifdef ESF
  if (smf->dTime-p->fTimeForm < smf->dESFEndTime+smf->dZAMSTime && smf->dTime-p->fTimeForm > smf->dZAMSTime)
  {
