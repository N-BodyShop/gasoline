...
void combDistDeletedGas(void *vp1,void *vp2)
{
	...
	if(!TYPETest((p1), TYPE_DELETED)) {
		...
		if (delta_m > 0) {
            ...
            if (mHot_new > 0) {
            printf("DELETEDBG3: iOrd %d %d mass:
                    %e %e massHot %e %e u %e %e uHot %e %e uDot %e %e nSmooth
                    %d\n", p1->iOrder, p2->iOrder, p1->fMass, p2->fMass,
                    p1->fMassHot, p2->fMassHot, p1->u, p2->u, p1->uHot, p2->uHot,
                    p1->uDot, p2->uDot, nSmooth);
...
{	
    ...
    if (p->fMassHot > 0)
    {
        ...
        for (i=0;i<nSmooth;++i) {
            ...
            q->fMass = m_new;
            q->fMassHot = mHot_new;
            if (!(p->fMassHot < p->fMass)) printf("DELETEDBG: iOrd %d %d mass:
                    %e %e massHot %e %e u %e %e uHot %e %e uDot %e %e nSmooth
