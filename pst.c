...
void pstKick(PST pst,void *vin,int nIn,void *vout,int *pnOut)
{
	...
	if (pst->nLeaves > 1) {
		...
		pkdKick(plcl->pkd,in->dvFacOne,in->dvFacTwo,
		    ...
		    in->iGasModel,in->z,in->duDotLimit, in->dTimeEnd, in->uhc);
...
void pstUpdateuDot(PST pst,void *vin,int nIn,void *vout,int *pnOut)
{
	...
	if (pst->nLeaves > 1) {
        ...
        pkdUpdateuDot(plcl->pkd,in->duDelta,in->dTime,in->z,in->uhc,in->iGasModel,in->bUpdateState);
		out->Time = pkdGetTimer(plcl->pkd,1);
		out->MaxTime = out->Time;
		out->SumTime = out->Time;
		out->nSum = 1;
...
void pstSphStep(PST pst,void *vin,int nIn,void *vout,int *pnOut)
{
	...
	if (pst->nLeaves > 1) {
		...
		pkdSphStep(plcl->pkd,in->dCosmoFac,in->dEtaCourant,in->dEtauDot,in->duMinDt,in->dDiffCoeff,in->dEtaDiffusion,in->dResolveJeans,in->bViscosityLimitdt,pdtMinGas);
		}
